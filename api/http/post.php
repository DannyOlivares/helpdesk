...
*/
    ...
    $app->post('/actividadesForzadas', function () use ($app) {
        $u = new Model\Escalamiento;
        return $app->json($u->actividadesForzadas());
    });

    $app->post('/forzarActividad', function () use ($app) {
        $u = new Model\Escalamiento;
        return $app->json($u->forzarActividad());
    });

    $app->post('/agregarGestionForzada', function () use ($app) {
        $u = new Model\Escalamiento;
        return $app->json($u->agregarGestionForzada());
    });

    $app->post('/todasLasActividades', function () use ($app) {
        $u = new Model\Escalamiento;
        return $app->json($u->actividadesAll());
    });
