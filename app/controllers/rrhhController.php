...
// Horas Extras---------------------------------------------------------------------------------------------------------------------
            ...
            case 'registra_horasextra':
                echo $this->template->render('rrhh/horasextra/ingreso_horas_extra', array(
                    ...
                    'horas_extras' => (new Model\Horasextra)->gethxtmp($this->user['id_user']),
                    'ultimo_id' => (new Model\Horasextra)->get_lastid($this->user['id_user'])
                ));
            break;
            case 'modificar':
...
//Turnos ------------------------------------------------------------------------------------------------------------------------
            ...
            case 'ver_turno_propio':
                ...
                echo $this->template->render('rrhh/turnos/turno_propio', array(
                    'menu_op' => $op,
                    'carga_turno'=>(new Model\Turnos)->GetTurnoPropio($this->user['rut_personal'],date('Y-m')),
                    'super'=>(new Model\Turnos)->GetSupervisorUsuario($this->user['id_super']),
                    'fechas_db'=>$fechas
                ));
            break;
            case 'revisar_turno_equipo_dia':
                echo $this->template->render('rrhh/turnos/turno_equipo_asignado', array(
                    'menu_op' => $op,
                    'getTurnoEquipoAsignado'=>(new Model\Turnos)->getTurnoEquipoAsignado($this->user['id_user'],date('Y-m-d')),
                    'getDatosEquipoAsignado'=>(new Model\Turnos)->getDatosEquipoAsignado($this->user['id_user'])
                ));
            break;
//-------------------------------------------------------------------------------------------------------------------------------
            case 'listar_anticipos_mensual':
                $actual = strtotime(date('d-m-Y'));
                $actual = strtotime("0 month", $actual);
                ...
                for ($i=0; $i<=$nmeses; $i++){
                    if ($i != 0 ) $actual = strtotime("-1 month", $actual);
                    $fechas[]=array('mesano_print' => date("M-Y", $actual),'mesano' => date("Y-m", $actual), 'ano' =>date("Y", $actual),'mes' =>date("m", $actual), 'annomes' =>date("Ym", $actual) );
                }

                $annomes=date('Ym');
                echo $this->template->render('rrhh/anticipos/listar_anticipo_mensual', array(
                    'menu_op' => $op,
                    'fechas_db'=>$fechas,
                    'anticipos_db' => (new Model\Rrhh)->getdbAnticiposMes($annomes)
                ));
            break;
            case 'validar_asistencia_mensual':
                $actual = strtotime(date('d-m-Y'));
                $actual = strtotime("1 month", $actual);
                $mesmenos = date("Y-m-d", strtotime("-4 month", $actual));
                $nmeses = (new Model\Varios)->diferencia_meses($mesmenos,date('Y-m-d'));
                ...
                echo $this->template->render('rrhh/asistencia/asistencia', array(
                    'menu_op' => $op,
                    'fechas_db'=>$fechas,
                    'getAllAsistenciaMensual'=>(new Model\Turnos)->getAllAsistenciaMensualDB($fecha_ini,$fecha_fin)
                ));
            break;
            case 'exportar_excel_listado_anticipos':
                (new Model\Rrhh)->exportar_excel_listado_anticipos();
            break;
            default:
