...
class Ausencias extends Models implements IModels {
    ...
    */
    ...
    public function getAllAusencias($fechadesde,$fechahasta){
        ...
        return  $this->db->query_select("select @rownum:=@rownum+1 as rownum,id_tblausencias,a.rut,tipo_ausencia,desde,hasta,fechamod,u.name,p.name nombres,usu_registra,observacion from (select @rownum:=0) r,(tblausencias a inner join users p on a.rut=p.rut_personal) inner join users u on a.usu_registra=u.id_user where (desde between '$fechadesde' and '$fechahasta') or (hasta between '$fechadesde' and '$fechahasta') order by desde,hasta");
    }
    ...
    public function validacion_eliminar() {
        ...
        $consulta=$this->db->select('*','tblausencias',"id_tblausencias='$idausencias' and usu_registra='".$this->user['id_user']."'","limit 1");
        if ($consulta==true){
    ...
    public function revisar_por_fecha(){
        ...
        if ($datosfecha === false){
            ...
            foreach ($datosfecha as $key => $value) {
                ...
                if ($value['usu_registra'] == $this->user['id_user']){
