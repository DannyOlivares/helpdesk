...
class Users extends Models implements IModels {
    ...
    private function authentication(string $email,string $pass) : bool {
        ...
        $query = $this->db->select('id_user,pass,estado,name','users',"email='$email' or rut_personal='$email' ",'LIMIT 1');
    ...
    */
    ...
    public function resetpass() : array {
        try {
            ...
            $fecha_reset = date ( 'Y-m-j',strtotime ( '+30 day' , strtotime ( date("Y-m-d") ) ));
    ...
    */
    public function lostpass() {
        try {
            ...
            $user_data = $this->db->select('id_user,name', 'users', "email='$email' or email_recepcion='$email'", 'LIMIT 1');
